...
const run = async()=>{
   ...
   let refFaceAiData = await faceapi.detectAllFaces(refFace).withFaceLandmarks().withFaceDescriptors() // face to check
   let facesToCheckAiData = await faceapi.detectAllFaces(facesToCheck).withFaceLandmarks().withFaceDescriptors() // in faces to check
   ...
   let faceMatcher = new faceapi.FaceMatcher(refFaceAiData) // facematcher function for checking the face
   ...
   facesToCheckAiData.forEach(face=>{
       const { detection, descriptor } = face // destructuring the detection and descriptor from element
